...
특히 엔드포인트 응답이 크고 필드가 많을 때는 전체를 다시 `refetch하지` 않고 변경된 일부 필드만 fresh하게 유지하고 싶을 수 있다.
...
최근 상세 페이지 내에서 유저가 작품에 대해 리액션을 등록할 수 있는 기능이 추가되었다.  
...
1. 네트워크 지연:
   느린 네트워크에서는 `/b` → `/c` 응답을 기다려야 하므로 UI 반응이 늦게 느껴진다.

2. 코드 복잡도 증가:
   `initialRating`과`stars` 를 따로 관리해야 하므로 코드 가독성이
   떨어진다.

3. 확장성 저하:
   비슷한 요구가 다른 필드에도 생기면 관리 로직이 점점 늘어나고, 복잡도가 높아진다.

## 접근 2: `setQueryData` 로 캐시 직접 수정 ( ✅ 채택 )

최종적으로는 [queryClient.setQueryData](https://tanstack.com/query/latest/docs/reference/QueryClient#queryclientsetquerydata) 를 사용해 `/a` 캐시에서 변경된 필드만 직접 업데이트하는 방식으로 리팩토링했다.
...
1. 즉시 반영:
   네트워크 요청 없이 응답값으로 바로 UI 업데이트 가능
...
- `/b` 응답에 **항상 최신 리액션 값이 포함됨이 보장된다.** (백엔드 개발자분과 확인함)
